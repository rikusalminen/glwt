...
struct painter
{
    ...
    pthread_cond_t surface_destroyed;
    int stopped, paused, visible, exposed, created, painting;
};
...
static void painter_init(struct painter *painter)
{
    ...
    pthread_cond_init(&painter->surface_destroyed, NULL);
...
static void painter_destroy(struct painter *painter)
{
    ...
    pthread_cond_destroy(&painter->surface_destroyed);
}
...
static int painter_wait(struct painter *painter)
{
    ...
    if(!painter->created && painter->painting)
        pthread_cond_broadcast(&painter->surface_destroyed);
...
static int painter_surface_created(struct painter *painter, int created)
{
    pthread_mutex_lock(&painter->lock);
    ...
    painter->created = created;

    if(!created) // must wait for painter thread finish before freeing the surface
    {
        LOGI("*** WAITING for painter to finish!\n");
        pthread_cond_wait(&painter->surface_destroyed, &painter->lock);
        LOGI("*** WAITING for painter..... DONE!\n");
    }
...
static void* painter_main(void *arg)
{
    ...
    while(painter_wait(&tw->painter) == 1)
    {
        ...
        float t = 0.5 + 0.5 * sinf(2.0 * M_PI * (framecount++ % 60) / 60.0);
        glClearColor(t * 0.2,t *  0.4,t *  0.7,t *  1.0);
...
static void win_event_callback(GLWTWindow *window, const GLWTWindowEvent *event, void *userdata)
{
    ...
    switch(event->type)
    {
        ...
        case GLWT_WINDOW_SURFACE_CREATE:
            painter_surface_created(&tw->painter, 1);
        ...
        case GLWT_WINDOW_SURFACE_DESTROY:
            painter_surface_created(&tw->painter, 0);
